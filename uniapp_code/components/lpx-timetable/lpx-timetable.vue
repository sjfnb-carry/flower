      ...
      <view class="course-container">
        <view class="week" v-for="(week, weekIndex) in courseData" :key="weekIndex">
          <view class="courseList" v-for="(course, courseIndex) in week" :key="courseIndex">
            <view @click="handleCourseClick(course, weekIndex, courseIndex)" class="course" :style="{ height: (course.length * 80) + 'px', background: course.backgroundColor }" v-if="course.length > 0">{{ course.name  }}
...
<script>
  export default {
	...
	// getTime(){
	// 		 let newArr=[];
	// 		  const res = this.$api.list('scheduletime', {page: 1,limit: 100});
	// 				res.data.list.map((item)=>{
	// 					 newArr.push({
	// 						index: `${item.sectionnum}`,
	// 						name: `${item.starttime}\n${item.endtime}`
	// 					})
	// 			})	
	// 			this.timetableType=newArr;
	// 		},
